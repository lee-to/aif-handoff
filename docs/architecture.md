...
The coordinator (`packages/agent/src/coordinator.ts`) uses a dual-trigger model: it polls via `node-cron` every 30 seconds as a fallback and also reacts to real-time events from the API WebSocket (task creation, moves, and explicit `agent:wake` signals). Duplicate wakes are debounced. A trigger received during an active cycle starts an overlapping cycle while global stage slots remain free; that cycle only picks up projects with no stage in flight, so project-local stage order is preserved. When no slots are free, the trigger is coalesced into one follow-up cycle instead. The interval tick dispatches the cycle without awaiting it — a cycle promise settles only after every stage it started has finished, so awaiting it would suppress every tick for the length of the longest stage and reduce the coordinator to a wake-only system. If the WebSocket is unavailable, the coordinator falls back to polling-only mode.
