...
| `INTERNAL_BROADCAST_TOKEN`                             | string  | _(unset)_                      | Shared secret for the internal broadcast endpoints the agent and MCP server use to emit WebSocket events. Without it those calls are only accepted from a loopback caller header while `NODE_ENV=development` — and `npm run dev` does not set `NODE_ENV`, so leaving it unset in local development means every broadcast is answered with `401`: the board stops live-updating and the coordinator's wake channel never fires on agent-side transitions. See [API reference](api.md#broadcast-task-update)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
