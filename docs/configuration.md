...
- Cron ticks and WebSocket wakes never wait for a running stage. A trigger received during an active cycle starts an overlapping cycle as long as global slots are free (up to 8 cycles in flight); that cycle skips every project that already holds a stage, so project-local stage order is preserved. Once `COORDINATOR_MAX_CONCURRENT_TASKS` is saturated, further triggers are coalesced into one follow-up cycle. This is what keeps a multi-hour implementer stage from stalling planners in other projects and from blocking the stale-task watchdog.
- The interval tick dispatches `pollAndProcess()` without awaiting it (`startPollScheduler(..., { awaitCallback: false })`). A poll cycle promise settles only after every stage it started has finished, so awaiting it in the scheduler would hold the scheduler's own re-entrancy guard for hours and silently drop every tick in between — leaving WebSocket wakes as the only trigger. Concurrency is bounded inside `pollAndProcess()` instead.
