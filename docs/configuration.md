...
| `AIF_AGENT_OVERLAPPING_POLL_CYCLES_ENABLED`            | boolean | `false`                        | Off-by-default rollout flag for overlapping coordinator poll cycles. When `false`, cron ticks and WebSocket wakes share one single-flight loop and the interval tick awaits the cycle. When `true`, a trigger received during an active cycle starts an overlapping cycle while global stage slots are free (up to 8 in flight, idle projects only), and the interval tick dispatches the cycle without awaiting it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- With `AIF_AGENT_OVERLAPPING_POLL_CYCLES_ENABLED=false` (default), cron ticks and WebSocket wakes share one single-flight poll loop. A wake received during an active cycle is coalesced into one follow-up cycle, preserving project-local stage order across trigger sources.
- With `AIF_AGENT_OVERLAPPING_POLL_CYCLES_ENABLED=true`, triggers never wait for a running stage. A trigger received during an active cycle starts an overlapping cycle as long as global slots are free (up to 8 cycles in flight); that cycle asks the database only for projects that hold no stage, so project-local stage order is preserved and busy projects never consume a lane slot an idle project could use. Once `COORDINATOR_MAX_CONCURRENT_TASKS` is saturated, further triggers are coalesced into one follow-up cycle. This is what keeps a multi-hour implementer stage from stalling planners in other projects and from blocking the stale-task watchdog.
- The same flag controls the interval tick. Enabled, it dispatches `pollAndProcess()` without awaiting it (`startPollScheduler(..., { awaitCallback: false })`): a poll cycle promise settles only after every stage it started has finished, so awaiting it in the scheduler would hold the scheduler's own re-entrancy guard for hours and silently drop every tick in between — leaving WebSocket wakes as the only trigger. Concurrency is bounded inside `pollAndProcess()` instead. Disabled, the tick awaits the cycle, which is the correct guard for a single-flight loop.
