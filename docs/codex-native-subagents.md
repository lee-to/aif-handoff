...
This rollout enables native Codex subagents as the default `useSubagents` strategy in AIF Handoff when the selected runtime is `codex` over the SDK transport.
...
- expected package version: `2.9.4` or later
...
- `codex` + `sdk` + `useSubagents=true` → native Codex subagents
...
6. Confirm logs show `usedNativeSubagentWorkflow: true`.
7. Confirm the prompt path does not downgrade to `$aif-*` unless `codexSubagentStrategy` is explicitly set to `isolated`.
