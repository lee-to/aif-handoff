...
| `AIF_QA_PIPELINE_ENABLED`                              | boolean | `false`                        | Master switch for task QA. When disabled, both `POST /tasks/:id/run-qa` and `POST /tasks/:id/run-qa-check` return `403`, and automatic QA is skipped. When enabled, `autoQa` generates change-summary/test-plan/test-cases; the separate `autoQaCheck` opt-in executes ready cases through `/aif-qa-check agent`. QA Check probes configured Playwright MCP state but blocks only browser-dependent cases when live browser tooling is unavailable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
