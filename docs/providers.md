...
- `codexSubagentStrategy` — `native` (default) or `isolated`; use `isolated` to force the legacy isolated skill-session fallback instead of native Codex subagent orchestration
